...
def get_data_feed():
  global CONFIG
  mixed_data_set = primefactors.factors(CONFIG.DATA_LIMIT, 1)
  data_sets = {
    train: split_data_and_labels(random.sample(data_main, CONFIG.SET_SIZE))
    validate: split_data_and_labels(random.sample(data_main, CONFIG.SET_SIZE))
    test: split_data_and_labels(random.sample(data_main, CONFIG.SET_SIZE))
  }

  return data_sets

def split_data_and_labels(mixedData):
...
def primeToArray(prime):
  global CONFIG
  outputSplitList = list(0.0 for d in range(0,CONFIG.MAX_DIGITS))
...
def fill_feed_dict(data_set, data_pl, labels_pl):
  ...
  Args:
    ...
    data_pl: The images placeholder, from placeholder_inputs().
  ...
  # `batch size` examples.
  data_feed, labels_feed = data_set.next_batch(CONFIG.BATCH_SIZE,
                                                 FLAGS.fake_data)
  feed_dict = {
      data_pl: data_feed,
...
def do_eval(sess,
  ...
  Args:
    ...
    data_placeholder: The images placeholder.
  ...
  steps_per_epoch = data_set.num_examples // CONFIG.BATCH_SIZE
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    logits = mnist.inference(data_placeholder,
                             FLAGS.hidden1,
                             FLAGS.hidden2)
